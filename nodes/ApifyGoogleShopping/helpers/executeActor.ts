...
import { ACTOR_ID } from '../ApifyGoogleShopping.node';
import { buildActorInput } from '../ApifyGoogleShopping.properties';
...
/**
 * Shape a single shopping result according to the chosen Output mode.
 ...
 * `shopping_results` array. Flatten to one product per n8n item.
 */
function flattenResults(items: any[]): Record<string, any>[] {
	const results: Record<string, any>[] = [];
	for (const item of items) {
		const all = Array.isArray(item?.shopping_results) ? item.shopping_results : [];
		for (const r of all) {
			results.push(r);
		}
	}
